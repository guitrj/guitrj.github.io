...
<body>
    ...
    <script type="module">
        ...
        controls.maxDistance = 80.0;   // [交互] 最大缩放距离 (允许拉远看星空)
            ...
            fragmentShader: `
                ...
                vec3 getStarField(vec3 dir) {
                    ...
                    for(float i=1.0; i<=2.0; i++) {
                        ...
                        float threshold = (i == 1.0) ? 0.40 : 0.09;
                ...
                vec4 getDiskColor(vec3 pos, float distToCenter) {
                    ...
                    // [物理] 开普勒旋转速度 (越近越快)
                    float speed = 2.5 / (distToCenter * 0.5 + 0.1); 
                    float rotAngle = angle + iTime * speed * 0.5;
                    
                    // 生成云雾噪点
                    float gas = fbm(vec2(distToCenter * 1.5, rotAngle * 2.0));
                    // 生成环状结构
                    float ringPattern = sin(distToCenter * 4.0 + gas * 2.0); 
                    ringPattern = smoothstep(-0.5, 0.8, ringPattern);

                    float intensity = (0.5 + 0.5 * gas) * (0.3 + 0.7 * ringPattern);
